...
if (!empty($_POST['cid']) && !empty($_POST['aid']) )
{
    ...
    $cid_query = "SELECT * FROM coaches WHERE cid = $cid";
    ...
    else 
    {
        ...
        $trains_command = "INSERT INTO trains(cid, aid) VALUES ($cid, $aid)";
        
		try
		{
			$result = mysqli_query($db, $trains_command);

			mysqli_query($db, "DELETE FROM trains WHERE aid = 0 OR cid = 0");
