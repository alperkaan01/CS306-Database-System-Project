...
if (!empty($_POST['mid']) && !empty($_POST['aid']) )
{
    ...
    $mid_query = "SELECT * FROM medic WHERE mid = $mid";
    ...
    {
        echo "One of Medic ID or Athlete ID you have entered was not found in database. Please try again.";
    }
    else 
    {
        ...
        $responsible_command = "INSERT INTO responsible_for(mid, aid) VALUES ($mid, $aid)";
        try
		{
			$result = mysqli_query($db, $responsible_command);

			mysqli_query($db, "DELETE FROM responsible_for WHERE aid = 0 OR mid = 0");
